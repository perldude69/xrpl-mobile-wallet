...
class LedgerUsb {
  ...
  Future<bool> hasPermission(UsbDevice usbDevice) {
    return LedgerUsbPlatform.instance.hasPermission(usbDevice);
  }
  ...
  Future<Map<String, dynamic>> connectionInfo() {
    return LedgerUsbPlatform.instance.connectionInfo();
  }

  /// Atomic APDU exchange on the open session (auto-reopens if needed).
  Future<Uint8List> exchangeApdu(
    Uint8List apdu, {
    String identifier = 'auto',
    int timeoutMs = 120000,
  }) {
    return LedgerUsbPlatform.instance.exchangeApdu(
      apdu,
